...
function showEdges(route){
    ...
    for(var j =  1; j < route.length ; j++ ){
        var edge = route[j]
        color = stationListModel.get(route[j].idFrom - 1).stationColor;
        ...
        if(edge.x3 != -1){
            ...
            circle.size = circle.oldsize*zoom
            circle.height = circle.width = circle.radius = circle.size
            circle.oldcolor = stationListModel.get(route[j-1].idFrom - 1).stationColor
            circle.color = stationListModel.get(route[j-1].idFrom - 1).stationColor;
            //circle.start(500)
            circles.push(circle)
            circle = circleComponent.createObject(map)
            ...
            circle.size = circle.oldsize*zoom
            circle.height = circle.width = circle.radius = circle.size
            circle.oldcolor = stationListModel.get(route[j].idTo - 1).stationColor
            circle.color = stationListModel.get(route[j].idTo - 1).stationColor;
            //circle.start(500)
            circles.push(circle)
            //console.log("circles");
        }
...
function showWay(route) {
    ...
    for(j =  1; j < route.length ; j++ ){
        ...
        var circle
        if(!mapModel.isTransition(route[j-1].id)){
            circle = circleComponent.createObject(map)
            circle.xold = lineOrX1;
            circle.yold = lineOrY1;
            circle.x = circle.xold*zoom;
            circle.y = circle.yold*zoom;
            circle.oldsize = 8;
            circle.size = circle.oldsize*zoom
            //circle.oldcolor = stationListModel.get(route[j-1].id - 1).stationColor
            circle.color = color
            //circle.start(500)
            circles.push(circle)
        }
        if(!mapModel.isTransition(route[j].id)){
            circle = circleComponent.createObject(map)
            circle.xold = lineOrX2;
            circle.yold = lineOrY2;
            circle.x = circle.xold*zoom;
...
function fill() {
    ...
    //var qpoint = new Object();
    //qpoint["x"] = 350
    //qpoint["y"] = 350
    //map.resizeContent(700,700,qpoint)
    //map.contentWidth = map.contentWidth*2
    //map.contentHeight = map.contentHeight*2
    //map.img.width = map.img.width*2
    //map.img.height = map.img.height*2
}
