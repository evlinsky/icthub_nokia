...
//            mapImage3.source = mapModel.getMapDir()+"3.jpg";
    ...
    Gps {
        ...
        onClosestStationChanged: {
            ...
            if (closestStation != -1)
                circle.visible = true
            var zoom = map.sourceSizeWidth/loader0.width
            circle.originalX = mapModel.getMapStationById(gps.closestStation+1)["x"]*(map.sourceSizeWidth/zoom)/mapModel.getXmlWidth()+((loader0.width)-(map.sourceSizeWidth/zoom))/2-circle.height/2
            circle.originalY = mapModel.getMapStationById(gps.closestStation+1)["y"]*(map.sourceSizeHeight/zoom)/mapModel.getXmlHeight()+((loader0.height)-(map.sourceSizeHeight/zoom))/2-circle.height/2
    ...
    function setCity(City){
        ...
        var oldDir = mapModel.getMapPastelDir()
        mapModel.setCity(City);
        StationMap.clearWay();

        if (oldDir != mapModel.getMapPastelDir()) {
