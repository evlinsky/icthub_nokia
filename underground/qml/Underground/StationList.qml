...
ListView {
    ...
    property variant fontSize: 7*main.coef
    ...
    header:  Rectangle {
        ...
        Text { x: 65; width: parent.width-x; height: parent.height; font.pointSize: stationListView.fontSize;
            verticalAlignment: Text.AlignVCenter; text: "Поиск станции"; wrapMode: Text.WordWrap}
        MouseArea {
            anchors.fill: parent
            onClicked: {
                stationSearch.stationSearch("")
                if (main.state=="stationList1")
                    main.state="stationSearch1"
                if (main.state=="stationList2")
                    main.state="stationSearch2"
            }
        }
    }
    ...
    delegate: Row {
        Rectangle {
            ...
            MouseArea {
                id: stationList
                anchors.fill: parent
                onClicked: ViewScripts.stationChoice(fullName, 0, stationColor);
            }
        }
    }
}
